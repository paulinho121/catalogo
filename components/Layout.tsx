...
export const Header: React.FC<{ setView: (v: AppView) => void; currentView: AppView }> = ({ setView, currentView }) => (
  <header className="fixed top-0 left-0 right-0 h-16 glass-morphism z-50 flex items-center justify-between px-6 border-b border-white/5">
    <div className="flex items-center gap-2 cursor-pointer" onClick={() => setView(AppView.HOME)}>
      <div className="w-8 h-8 bg-brand-gradient rounded-lg flex items-center justify-center text-brand-dark font-bold">C</div>
      <h1 className="text-xl font-bold text-slate-100 tracking-tight">Catalogo<span className="text-brand-primary">AI</span></h1>
    </div>
    <nav className="hidden md:flex items-center gap-6">
      <button
        onClick={() => setView(AppView.HOME)}
        className={`text-sm font-medium transition-colors ${currentView === AppView.HOME ? 'text-brand-primary' : 'text-slate-400 hover:text-slate-100'}`}
      >
        Dashboard
      </button>
      <button
        onClick={() => setView(AppView.CHAT)}
        className={`text-sm font-medium transition-colors ${currentView === AppView.CHAT ? 'text-brand-primary' : 'text-slate-400 hover:text-slate-100'}`}
      >
        Consultar
      </button>
      <button
        onClick={() => setView(AppView.REGISTER)}
        className={`text-sm font-medium transition-colors ${currentView === AppView.REGISTER ? 'text-brand-primary' : 'text-slate-400 hover:text-slate-100'}`}
      >
        Cadastrar
      </button>
      <button
        onClick={() => setView(AppView.ADMIN)}
        className={`text-sm font-medium transition-colors ${currentView === AppView.ADMIN ? 'text-brand-primary' : 'text-slate-400 hover:text-slate-100'}`}
      >
        Gestão
      </button>
    </nav>
  ...
  return (
    <div className="min-h-screen bg-brand-dark flex flex-col text-slate-200">
      ...
      <footer className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-slate-200 flex items-center justify-around px-4 z-50">
        ...
        <button onClick={() => setView(AppView.CHAT)} className={`flex flex-col items-center gap-1 ${currentView === AppView.CHAT ? 'text-brand-primary' : 'text-slate-500'}`}>
        ...
        <button onClick={() => setView(AppView.VOICE)} className={`flex flex-col items-center gap-1 ${currentView === AppView.VOICE ? 'text-brand-primary' : 'text-slate-500'}`}>
          <div className="w-10 h-10 bg-brand-gradient -mt-8 rounded-full flex items-center justify-center text-brand-dark shadow-lg shadow-brand-primary/20">
        ...
        <button onClick={() => setView(AppView.ADMIN)} className={`flex flex-col items-center gap-1 ${currentView === AppView.ADMIN ? 'text-brand-primary' : 'text-slate-500'}`}>
