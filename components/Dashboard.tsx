    ...
    <div className="max-w-7xl mx-auto p-4 md:p-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        <div className="animate-slideDown">
          <h2 className="text-4xl font-extrabold text-slate-900 tracking-tight">Painel de <span className="text-blue-600">Conhecimento</span></h2>
          <p className="text-slate-500 mt-2 text-lg">Central de inteligência para o seu catálogo técnico.</p>
        </div>
        <div className="flex gap-3 bg-white p-1.5 rounded-2xl shadow-sm border border-slate-100">
          <div className="px-4 py-2 text-center border-r border-slate-100">
            <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest">Manuais</span>
            <span className="text-lg font-bold text-slate-800">{MOCK_DOCUMENTS.length}</span>
          </div>
          <div className="px-4 py-2 text-center">
            <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest">SKUs</span>
            <span className="text-lg font-bold text-slate-800">{MOCK_PRODUCTS.length}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
        <button
          ...
          className="group relative overflow-hidden bg-blue-600 p-10 rounded-[2.5rem] text-left text-white shadow-2xl shadow-blue-200 transition-all hover:-translate-y-2"
        >
          ...
          <p className="text-blue-100 text-sm leading-relaxed opacity-90">Consulte dados complexos e comparações técnicas em linguagem natural.</p>
          <div className="mt-8 flex items-center gap-2 text-xs font-bold text-blue-200 uppercase tracking-widest group-hover:translate-x-2 transition-transform">
        ...
        <button
          ...
          className="group bg-white p-10 rounded-[2.5rem] text-left shadow-xl shadow-slate-200/50 border border-slate-50 transition-all hover:-translate-y-2"
        >
          ...
          <h3 className="text-2xl font-bold text-slate-800 mb-3">Voz em Campo</h3>
          <p className="text-slate-500 text-sm leading-relaxed">Interação viva e sem mãos. Ideal para manutenção em campo e chão de fábrica.</p>
          <div className="mt-8 flex items-center gap-2 text-xs font-bold text-blue-600 uppercase tracking-widest group-hover:translate-x-2 transition-transform">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2">
          <div className="flex justify-between items-center mb-8">
            <h4 className="text-2xl font-bold text-slate-800 tracking-tight">Destaques do Catálogo</h4>
            <button className="text-blue-600 font-semibold text-sm hover:underline">Ver catálogo completo</button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {MOCK_PRODUCTS.length === 0 ? (
              <div className="col-span-2 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2rem] p-12 text-center">
                <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-4 text-slate-400">
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" /></svg>
                </div>
                <h3 className="text-lg font-bold text-slate-700 mb-2">Seu catálogo está vazio</h3>
                <p className="text-slate-500 max-w-md mx-auto mb-6">Comece cadastrando seus produtos e manuais para alimentar a inteligência artificial.</p>
                <button
                  onClick={() => setView(AppView.REGISTER)}
                  className="px-6 py-3 bg-blue-600 text-white font-bold rounded-xl shadow-lg shadow-blue-200 hover:bg-blue-700 hover:scale-105 transition-all"
                >
              ...
              MOCK_PRODUCTS.map((prod) => (
                <div key={prod.id} className="group bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 flex gap-6 transition-all hover:shadow-xl hover:shadow-blue-500/5 hover:-translate-y-1">
                  <div className="w-32 h-32 rounded-2xl overflow-hidden bg-slate-50 shrink-0 shadow-inner group-hover:scale-105 transition-transform">
                    <img src={prod.imageUrl} alt={prod.name} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 flex flex-col">
                    <div className="flex justify-between items-start">
                      <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-md uppercase tracking-wider">{prod.category}</span>
                      {prod.isFavorite && (
                    ...
                    <h5 className="font-bold text-slate-800 text-lg mt-2 group-hover:text-blue-600 transition-colors">{prod.name}</h5>
        ...
        <div>
          <h4 className="text-2xl font-bold text-slate-800 tracking-tight mb-8">Saúde do Sistema AI</h4>
          <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
            <div className="space-y-8">
              <div>
                <div className="flex justify-between items-end mb-2">
                  <span className="text-sm font-bold text-slate-700">Indexação RAG</span>
                  <span className="text-xs font-bold text-blue-600">85%</span>
                </div>
                <div className="h-2 w-full bg-slate-50 rounded-full overflow-hidden border border-slate-100">
                  <div className="h-full bg-blue-600 rounded-full w-[85%] animate-pulse"></div>
                </div>
                <p className="text-[10px] text-slate-400 mt-2 font-medium italic">Processando "Guia de Bombas v3.pdf"...</p>
              </div>

              <div className="pt-6 border-t border-slate-100">
                <span className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Atividade Recente</span>
                <div className="space-y-4">
                  {[
                    ...
                    <div key={i} className="flex gap-3">
                      ...
                      <div>
                        <p className="text-xs text-slate-700 font-semibold">{act.action}</p>
                        <p className="text-[10px] text-slate-400">{act.user} • {act.time} atrás</p>
                      </div>
                    </div>
                  ))}
                </div>
