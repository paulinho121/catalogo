    ...
    <div className="max-w-6xl mx-auto p-4 md:p-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm">Gerencie os arquivos que alimentam a IA do seu catálogo.</p>
        </div>
        <label className="bg-blue-600 text-white px-6 py-2.5 rounded-xl font-medium cursor-pointer hover:bg-blue-700 transition-colors flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <span className="text-slate-400 text-xs font-bold uppercase tracking-wider">Total indexado</span>
          <p className="text-3xl font-bold text-slate-800 mt-1">128 MB</p>
          <div className="mt-4 h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
            <div className="h-full bg-blue-600 w-3/4"></div>
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <span className="text-slate-400 text-xs font-bold uppercase tracking-wider">Consultas (30d)</span>
          <p className="text-3xl font-bold text-slate-800 mt-1">2.4k</p>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <span className="text-slate-400 text-xs font-bold uppercase tracking-wider">Taxa de Acerto RAG</span>
          <p className="text-3xl font-bold text-slate-800 mt-1">98.2%</p>
          <p className="text-slate-400 text-xs mt-2 italic">Baseado em feedback do usuário</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-slate-50 border-b border-slate-100">
            <tr>
              <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase">Arquivo</th>
              <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase">Tamanho</th>
              ...
              <tr key={doc.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${
                    doc.status === 'indexed' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700 animate-pulse'
                  }`}>
